  ...
  return (
    <aside className={cn("w-72 glass flex flex-col border-r border-slate-200 dark:border-white/5", className)}>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar">
        ...
        <div>
          <h3 className="text-[11px] font-bold text-emerald-600 dark:text-emerald-400/70 uppercase tracking-widest mb-3">
            Medical Core
          </h3>
          <div className="space-y-2">
            {fieldTypes.slice(5).map((field) => (
        ...
        <div className="mt-8 p-4 rounded-xl border border-medical-primary/20 bg-medical-primary/5 relative overflow-hidden group">
          ...
          <div className="flex items-center gap-2 mb-2">
            <div className="w-2 h-2 bg-medical-neon rounded-full" />
            <h4 className="text-[10px] font-bold text-slate-800 dark:text-white uppercase tracking-wider">
              AI Assistant
            </h4>
          </div>
          <p className="text-[10px] text-slate-600 dark:text-slate-400 italic">
...
const DraggableItem: React.FC<{
  ...
  fullWidth?: boolean;
  rows?: number;
}> = ({ type, label, icon, color, fullWidth, rows }) => {
  const { activeTemplate, addField, activeSectionId } = useStore();
  ...
  return (
    ...
    >
      ...
      <span
        className={cn(
          "text-[11px] font-semibold text-slate-700 dark:text-slate-300 group-hover:text-slate-900 dark:group-hover:text-white transition-colors",
          fullWidth ? "" : "leading-tight"
        )}
      >
        {label}
      </span>
